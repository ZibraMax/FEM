					...
					for j in range(o):
						for k in range(len(e.Z)):
							for knl in range(len(enl.Z)):
								...
								Kuu[i,j] += azn*self.t[ee]*self.t[eenl]*(self.C11[ee]*dpx[k,0,i]*dpxnl[k,0,j]+self.C66[eenl]*dpx[k,1,i]*dpxnl[k,1,j])*detjac[k]*e.W[k]*detjacnl[knl]*enl.W[knl]
								Kuv[i,j] += azn*self.t[ee]*self.t[eenl]*(self.C12[ee]*dpx[k,0,i]*dpxnl[k,1,j]+self.C66[ee]*dpx[k,1,i]*dpxnl[k,0,j])*detjac[k]*e.W[k]*detjacnl[knl]*enl.W[knl]
								Kvu[i,j] += azn*self.t[ee]*self.t[eenl]*(self.C12[ee]*dpx[k,1,i]*dpxnl[k,0,j]+self.C66[ee]*dpx[k,0,i]*dpxnl[k,1,j])*detjac[k]*e.W[k]*detjacnl[knl]*enl.W[knl]
								Kvv[i,j] += azn*self.t[ee]*self.t[eenl]*(self.C11[ee]*dpx[k,1,i]*dpxnl[k,1,j]+self.C66[ee]*dpx[k,0,i]*dpxnl[k,0,j])*detjac[k]*e.W[k]*detjacnl[knl]*enl.W[knl]
				subm = np.linspace(0, 2*m-1,2*m).reshape([2, m]).astype(int)
