"""2D Elasticity: Orthotropic Plane Stress
"""
...
class PlaneStressOrthotropic(Core):
    ...
    def __init__(self, geometry: Geometry, E1: Tuple[float, list], E2: Tuple[float, list], G12: Tuple[float, list], v12: Tuple[float, list], t: Tuple[float, list], fx: Callable = lambda x: 0, fy: Callable = lambda x: 0) -> None:
            ...
            for i in range(m):  # self part must be vectorized
                for j in range(m):
                    for k in range(len(e.Z)):  # Iterate over gauss points on domain
                        ...
                        Kvv[i, j] += (self.C22[ee]*dpx[k, 1, i]*dpx[k, 1, j] +
