if __name__ == '__main__':
    ...
    # O.solve()
    solutions = json.load(
        open('./Examples/Mesh_tests/Bar_and_hinge_non_linear_loco.json'))
    ...
    ax2.set_xlabel('Vertical displacement')
    ax2.legend()
    ax2.grid()
    ...
    def animate(i, lines):
        ...
        for j, cosa in enumerate(cosas):
            cosa.set_data(displacements[:(i+1), j], load_factors[:(i+1), j])
        return lines+cosas
    ...
    html = anim.to_jshtml()
    with open('./mori_debugged.html', 'w') as f:
