if __name__ == '__main__':
    ...
    from FEM.Elements.E0D.Spring import Spring
    ...
    types = ['MITC6']*(len(elements))

    geo = Geometry3D(elements, coords, types, 5, fast=True)
    gdl = geo.elements[1].gdl
    springs = []

    springs.append(Spring(gdl[-2, 0], geo.ngdl, kf))
    springs.append(Spring(gdl[-2, 1], geo.ngdl+1, kf))
    springs.append(Spring(gdl[-2, 3], geo.ngdl+2, kf))
    gdl[-2, 0] = geo.ngdl + 0
    gdl[-2, 1] = geo.ngdl + 1
    gdl[-2, 3] = geo.ngdl + 2
    geo.ngdl += 3
    ...
    geo.elements.append(springs[0])
    geo.elements.append(springs[1])
    geo.elements.append(springs[2])
    for node in [0, 1, 2, 4, 5, 6]:
        geo.cbe += [[node*5, 0], [node*5+1, 0],
                    [node*5+2, 0], [node*5+3, 0], [node*5+4, 0]]
    O = ShellAndHingeNonLinear(geo, EA, 0.3, 0.01, verbose=False)
    ...
    plt.tight_layout()
    for e in O.elements:
        if e.__class__.__name__ != 'Spring':
